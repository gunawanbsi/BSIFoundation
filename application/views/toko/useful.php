    ...
    <div class="row-toko text-center ms-l">
		<?php foreach ($useful as $u) : ?>
			<div class="card-toko mb-3 ms-3" style="width: 20rem;">
				<img src="<?php echo base_url().'/assets/img/upload/'.$u->gambar ?>" class="card-img-top" alt="...">
				<div class="card-body">
					<h5 class="card-title"><?php echo $u->barang ?></h5>
					<p> <?php echo $u->keterangan ?> </p>
					<span class="badge"> Rp. <?php echo number_format($u->harga, 0, ',', '.') ?></span>
					<?php echo anchor('toko/tambahKeranjang/' .$u->id, '<div class="button-green mb-3 mt-3">Tambah ke Keranjang </div>') ?>
					<?php echo anchor('toko/detailBarang/' .$u->id, '<div class="button-blue mb-3 mt-3">Detail </div>') ?>
				</div>
			</div>
		<?php endforeach ?>
	</div>
